...
class FFN(nn.Module):
  def __init__(self, in_channels, out_channels, filter_channels, kernel_size, p_dropout=0., activation=None, causal=False):
    ...
    self.conv_1 = nn.Sequential(
        ...
        nn.Conv1d(in_channels, filter_channels, 1)
    )
    #conv_2 = nn.Conv1d(filter_channels, out_channels, kernel_size)
    self.conv_2 = nn.Sequential(
        nn.Conv1d(filter_channels, filter_channels, kernel_size, groups=filter_channels),
        nn.Conv1d(filter_channels, out_channels, 1)
    )
    self.drop = nn.Dropout(p_dropout)
